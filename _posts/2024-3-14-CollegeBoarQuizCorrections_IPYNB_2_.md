...
## HACK:
What you can dto make the maximum value leading to an overflow error is make the binary 1111111111, which results in an overflow because it cannot represent values of that length. And we also can get a underflow error subtract 000000001 from 0 which results in 255.
...
for A in [False, True]:
    for B in [False, True]:
        for C in [False, True]:
            print(A, B, C, "-->", burglar_alarm(A, B, C))
...
# Define a list of favorite colors in binary format
rgb_favorites = [('10101010', '01010101', '10101010'),  # Gray
                 ('11110000', '00001111', '00000000'),  # Red
                 ('00000000', '11110000', '00001111'),  # Green
                 ('00001111', '00000000', '11110000'),  # Blue
                 ('11111111', '11111111', '00000000')]  # Yellow

# Plot the favorite colors
plot_colors(rgb_favorites)
rgb_favorites = [('10101010', '01010101', '10101010'),  # Gray
                 ('11110010', '00001101', '00000000'),  # Red
                 ('00000000', '11110000', '00011111'),  # Green
                 ('00001111', '00000000', '11100000'),  # Blue
                 ('11111111', '11111111', '00000000')]  # Yellow
...
![png](output_8_3.png)
    ...
    binary: 10101010 01010101 10101010
    decimal 170 85 170
    proportion 0.6666666666666666 0.3333333333333333 0.6666666666666666
    binary: 11110000 00001111 00000000
    decimal 240 15 0
    proportion 0.9411764705882353 0.058823529411764705 0.0
    binary: 00000000 11110000 00001111
    decimal 0 240 15
    proportion 0.0 0.9411764705882353 0.058823529411764705
    binary: 00001111 00000000 11110000
    decimal 15 0 240
    proportion 0.058823529411764705 0.0 0.9411764705882353
    binary: 11111111 11111111 00000000
    decimal 255 255 0
    proportion 1.0 1.0 0.0
...
# O(n^3) Algorithm that accesses each element in the list n times, n * n times
def algorithm_cubed(lst):
    for i in lst:
        for j in lst:
            for x in lst:
                pass
...
# O(2^n) Algorithm that generates Fibonacci sequence recursively
...
# Measure the time taken by the recursive Fibonacci algorithm
start = time.time()
fibonacci_recursive(20)  # Adjust the argument as needed
end = time.time()
print(f"Recursive Fibonacci took {(end - start)*1000:.2f} milliseconds")

```

    Algorithm 2 * N took 0.13 milliseconds
    Algorithm N^2 took 376.78 milliseconds
    ...
    Cell In[8], line 59
         57 # Measure the time taken by algorithm3
         58 start = time.time()
    ---> 59 algorithm_cubed(lst)
         60 end = time.time()
         61 print(f"Algorithm 10 times took {(end - start)*1000:.2f} milliseconds")


    Cell In[8], line 28, in algorithm_cubed(lst)
         26 for j in lst:
         27     for x in lst:
    ---> 28         pass
...
def version_I(idList):
    ...
    topScore = 0
    for idNum in idList:
        predictedScore = GetPrediction(idNum)
        if predictedScore > topScore:
            topScore = predictedScore
    end = time.time()
    print(f"Version I took {end - start:.2f} seconds")

def version_II(idList):
    start = time.time()
    topID = idList[0]
    for idNum in idList:
        if GetPrediction(idNum) > GetPrediction(topID):
            topID = idNum
    end = time.time()
...
```python
def multiply(x, y):
    # Ensure y is positive for the loop
    y_positive = abs(y)
    count = 0
    result = 0
    while count < y_positive:
...
```python
animal = "jackrabbit"[0:4]  # Substring("jackrabbit", 0, 4)
animal += "a"  # Concat(animal, "a")
animal = "antelope"[4:8] + animal  # Concat(Substring("antelope", 4, 8), animal)
